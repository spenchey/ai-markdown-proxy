...
## Corpus Check
- 35 files · ~21,779 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 359 nodes · 688 edges · 36 communities (27 shown, 9 thin omitted)
...
1. `Site` - 22 edges
2. `resolve_site()` - 19 edges
3. `AgentAccessTests` - 19 edges
...
- `InvalidRequest` --inherits--> `ValueError`  [EXTRACTED]
  agent_access.py →   _Bridges community 1 → community 29_
- `resolve_site()` --references--> `Site`  [EXTRACTED]
  server.py → server.py  _Bridges community 0 → community 7_
- `serve_agent_query()` --calls--> `parse_agent_query()`  [EXTRACTED]
  server.py → server.py  _Bridges community 29 → community 7_
...
Cohesion: 0.09
Nodes (63): agent_query_health(), api_inventory_detail(), api_inventory_search(), build_query_documents(), build_static_query_documents(), cache_get(), cache_set(), canonical_catalog_link() (+55 more)
...
Cohesion: 0.15
Nodes (32): Response, agent_query_error(), api_error(), api_response(), cache_stats(), gbp_review_operations(), gbp_review_operations_html(), health() (+24 more)
